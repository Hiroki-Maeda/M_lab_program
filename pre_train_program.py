...
for j in range(len(words)):
	...
	for k in range(len(ch_names)):
		...
		(data_num, temp_data_length) = filted_data.shape
		
		
		if j == 0 and k == 0:
			all_filted_data = np.zeros((word_num*b_num,round(data_num*(classification_modelnum_rate+test_num_rate)),temp_data_length))
		####
