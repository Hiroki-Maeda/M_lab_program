...
% AUTHOR: Jimmy Chui
% LAST REVISION: July 30, 2003
%
% REVISION NOTES
% 17.04.13 - Florian removed loops from the code to speed up this function considerably (~40 times?)
...
function [RESULT1, RESULT2] = lab_SL(S_in,lag,m,w1,w2,pref,speed,option)

% time results
time1 = cputime;
time2 = 0;

% adapted from synchronization(.) from unit6.pas

% check number of input arguments
error( nargchk( 7, 8, nargin ) ); % ensure there are 7 (+1 optional) arguments
...
% standard output & standard error fid, used with fprintf
%STDOUT = 1;
%STDERR = 2;
...
if option ~= 's'
    ...
    fprintf(2,'Data\n');
    fprintf(2,'%d channels, %d samples\n',num_chans,num_samples);
    fprintf(2,'\n');
    
    % output values of some variables
    fprintf(2,'Parameters\n');
    fprintf(2,'lag = %d, embedding dimension m = %d\n',lag,m);
    fprintf(2,'window w1 = %d, w2 = %d\n',w1,w2);
    fprintf(2,'pref = %.2f\n',pref);
    fprintf(2,'speed factor: %d\n',speed);
    fprintf(2,'\n');
end %if
...
if (w1 > maxw1 )
    if maxw1 >= 0
        fprintf(2,'    w1 is too large. From your chosen m1, m2, lag and m, w1 must be <= %d\n',floor((m2-lag*(m-1)-m1-1)/2) );
    else
...
%for i = speed:speed:(m2 - lag*(m-1))  %<- was wrong in original code
for i = 1:speed:(m2 - lag*(m-1))
    i_count = i_count + 1;
    
    for j = (pm+1):floor(i_count/num_it*pmdots)
        if option ~= 's'
            fprintf(2,'.'); % character to display
        end
    end %for
...
if option ~= 's'
    ...
    fprintf(2,'overall synchronization likelihood: %f\n', S);
    fprintf(2,'elapsed time: %.3f seconds\n', time2-time1);
end %if
