...
for j in range(len(words)):
	...
	for k in range(len(ch_names)):
		...
		(temp_data_num, temp_data_length) = filted_data.shape
		
		
		if j == 0 and k == 0:
			all_filted_data = np.zeros((word_num*b_num,round(temp_data_num*(classification_modelnum_rate+test_num_rate)),temp_data_length))
		####
		all_filted_data[j*b_num+k] = filted_data[round(temp_data_num*hmm_trainnum_rate):,:]	
		data_div_state_0[k].extend(filted_data[0:round(temp_data_num*hmm_trainnum_rate),0:24].reshape(-1,).tolist())		
		for i in range(one_state_num):
			data_div_state[j][k][i].extend(filted_data[0:round(temp_data_num*hmm_trainnum_rate),24+i*225:24+i*225+225].reshape(-1,).tolist())		
